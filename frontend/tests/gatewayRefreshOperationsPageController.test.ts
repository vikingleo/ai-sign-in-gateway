...
import { useGatewayRefreshOperationsPageActions } from '../src/gatewayRefreshOperationsPageController.ts'
...
const refreshOperationsPageControllerPath = new URL('../src/gatewayRefreshOperationsPageController.ts', import.meta.url)
...
test('useGatewayRefreshOperationsPageActions wires catalog and manual refresh actions', async () => {
  const events: string[] = []
  const routes = ref([route({ id: 11, site_id: 101 })])
  const siteGroups = ref<SiteGroup[]>([])
  const routeGroups = ref<GatewayRouteGroup[]>([])

  const actions = useGatewayRefreshOperationsPageActions({
    routes,
  ...
  assert.deepEqual(siteGroups.value, [{ name: '生产', count: 1 }])
...
test('GatewayView delegates refresh operation wiring to the refresh operations page controller', async () => {
  ...
  assert.ok(operationsControllerSource.includes("useGatewayPageRefreshActions"), "GatewayView delegates refresh operation wiring to the page refresh actions controller should keep useGatewayPageRefreshActions in gateway page controller")
  assert.ok(pageRefreshActionsControllerSource.includes("useGatewayRefreshOperationsPageActions"), "GatewayView delegates refresh operation wiring to the refresh operations page controller should keep useGatewayRefreshOperationsPageActions in gateway page refresh actions controller")
  assert.ok(shellBindingsControllerSource.includes("refreshActions.handleRefresh"), "GatewayView delegates refresh operation wiring to the refresh operations page controller should keep refreshActions.handleRefresh in gateway page controller")
})
